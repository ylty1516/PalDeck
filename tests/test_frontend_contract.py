...
def test_palschema_import_type_and_card_label_are_exposed():
    html = HTML.read_text(encoding="utf-8")
    render = RENDER.read_text(encoding="utf-8")

    assert '<option value="palschema">PalSchema 内容模组</option>' in html
    assert 'mod.mod_type === "palschema" ? "PalSchema"' in render
