...
def test_palschema_identity_supports_both_ue4ss_layouts(tmp_path: Path):
    store = IgnoredModStore(tmp_path / "ignored-mods-v1.json")
    nested = IgnoredIdentity(
        "PalSchema", "palschema_nested", "Hex Reworked Shop 3b"
    )
    classic = IgnoredIdentity(
        "palschema", "palschema_classic", "BetterOilPumps"
    )

    store.add(GAME_A, nested)
    store.add(GAME_A, classic)
    ...
    assert set(store.list(GAME_A)) == {nested, classic}
