...
def test_ue4ss_cpp_main_dll_is_detected_before_nested_palschema_content(
    tmp_path: Path,
) -> None:
    archive = make_zip(
        tmp_path / "palschema-framework.zip",
        {
            "PalSchema/dlls/main.dll": b"dll",
            "PalSchema/mods/Example/raw/example.json": b"{}",
        },
    )
    ...
    assert result.kind is ModKind.UE4SS
    assert result.display_name == "PalSchema"
    assert result.content_root == (tmp_path / "out" / "PalSchema").resolve()


def test_palschema_content_pack_is_detected_from_supported_folder(tmp_path: Path) -> None:
    archive = make_zip(
        tmp_path / "schema-mod.zip",
        {
            "Better Oil Pumps/metadata.json": b'{"name":"Better Oil Pumps"}',
            "Better Oil Pumps/raw/change.jsonc": b"{}",
            "Better Oil Pumps/blueprints/pump.json": b"{}",
        },
    )

    result = inspect_and_extract(archive, tmp_path / "out")

    assert result.kind is ModKind.PALSCHEMA
    assert result.display_name == "Better Oil Pumps"
    assert result.content_root == (tmp_path / "out" / "Better Oil Pumps").resolve()
    assert result.groups == ()


def test_generic_paks_wrapper_is_not_misclassified_as_palschema(tmp_path: Path) -> None:
    archive = make_zip(tmp_path / "regular.zip", {"Wrapper/Paks/Cool.pak": b"pak"})
...
def test_palschema_paks_only_pack_requires_metadata_signal(tmp_path: Path) -> None:
    archive = make_zip(
        tmp_path / "schema-paks.zip",
        {
            "SchemaPak/metadata.json": b'{"name":"SchemaPak"}',
            "SchemaPak/paks/content.pak": b"pak",
        },
    )
    ...
    assert result.kind is ModKind.PALSCHEMA
    assert result.display_name == "SchemaPak"


def test_multiple_palschema_content_packs_are_rejected(tmp_path: Path) -> None:
    archive = make_zip(
        tmp_path / "multiple.zip",
        {"First/raw/a.json": b"{}", "Second/raw/b.json": b"{}"},
    )

    assert_rejected(archive, tmp_path / "out", "多个 PalSchema")
