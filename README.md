...
从 [Releases](https://github.com/ylty1516/PalDeck/releases) 下载 `PalDeck-v2.3.1-windows-portable.zip`，完整解压后双击 `PalDeck-portable/PalDeck.exe`。建议安装 [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)；Windows 11 通常已自带。
...
- `dist/PalDeck-v2.3.1-windows-portable.zip`
- `dist/PalDeck-v2.3.1-windows-portable.zip.sha256`

发布 GitHub Release 时，必须同时上传便携 ZIP 与构建生成的同名 `.sha256`（例如 `PalDeck-v2.3.1-windows-portable.zip` 和 `PalDeck-v2.3.1-windows-portable.zip.sha256`）；缺少或不匹配时应用会安全拒绝更新。构建脚本不会复制文件到桌面。构建完成后可运行正式冒烟脚本；它使用 F 盘 fresh data/TEMP，启动真实 EXE 和 WebView，并通过内部 HTTP 会话验证五个视图、三套主题、三种樱花风格、内置 UE4SS 来源/摘要界面标记、Workshop 空状态及默认背景，全程不写入真实游戏目录：
