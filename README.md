...
从 [Releases](https://github.com/ylty1516/PalDeck/releases) 下载 `PalDeck-v2.3.5-windows-portable.zip`，完整解压后双击 `PalDeck-portable/PalDeck.exe`。建议安装 [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)；Windows 11 通常已自带。
...
- 提供逐文件 Mod 健康报告、损坏清单备份恢复、内容寻址健康副本和可预演修复；缺失文件可无损恢复，修改文件会先隔离并在确认后替换。
- 设置页内置 Mod 健康中心，可批量建立健康副本并执行不会覆盖用户修改的安全自动修复。
- 游戏故障恢复会记录 Palworld 的运行与退出会话；由用户确认本次是否正常，标记故障后可回滚相对上次稳定状态中新启用的本地或 Workshop Mod。PalDeck 不会把普通退出猜成崩溃，也不会在游戏运行中修改文件。
...
- `dist/PalDeck-v2.3.5-windows-portable.zip`
- `dist/PalDeck-v2.3.5-windows-portable.zip.sha256`

发布 GitHub Release 时，必须同时上传便携 ZIP 与构建生成的同名 `.sha256`（例如 `PalDeck-v2.3.5-windows-portable.zip` 和 `PalDeck-v2.3.5-windows-portable.zip.sha256`）；缺少或不匹配时应用会安全拒绝更新。构建脚本不会复制文件到桌面。构建完成后可运行正式冒烟脚本；它使用 F 盘 fresh data/TEMP，启动真实 EXE 和 WebView，并通过内部 HTTP 会话验证五个视图、三套主题、三种樱花风格、内置 UE4SS 来源/摘要界面标记、Workshop 空状态及默认背景，全程不写入真实游戏目录：
