...
从 [Releases](https://github.com/ylty1516/PalDeck/releases) 下载 `PalDeck-v2.3.0-windows-portable.zip`，完整解压后双击 `PalDeck-portable/PalDeck.exe`。建议安装 [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)；Windows 11 通常已自带。
...
- 首次读取会发现安装 PalDeck 前已有的 `~mods`、`LogicMods` 以及 classic/nested 两种 UE4SS 布局，并标记为“外部发现”；可取消管理并保留游戏文件，也可在设置中重新发现。
