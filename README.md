...
从 [Releases](https://github.com/ylty1516/PalDeck/releases) 下载 `PalDeck-v2.2.2-windows-portable.zip`，完整解压后双击 `PalDeck-portable/PalDeck.exe`。建议安装 [Microsoft Edge WebView2 Runtime](https://developer.microsoft.com/microsoft-edge/webview2/)；Windows 11 通常已自带。

应用内自更新源暂保持为旧仓库 `ylty1516/palworld-mod-manager` 的 GitHub Release，并在 SHA-256 验证后于退出时替换当前程序；不接受客户端提供的下载 URL。可识别新的 `PalDeck.exe` / PalDeck 便携 ZIP，旧版 `PalMod.exe` 资源仍保持兼容。
...
- 首次读取会接管安装 PalDeck 前已有的 `~mods`、`LogicMods` 以及 classic/nested 两种 UE4SS 布局；“刷新模组”会执行真实磁盘重扫。
- 扫描 Steam 客户端已订阅并下载的 Workshop 项目，显示来源、作者、版本、依赖和安装类型，并管理启停状态。
- 提供由 [Okaetsu/RE-UE4SS](https://github.com/Okaetsu/RE-UE4SS) 维护的固定 Palworld 专用 UE4SS 内置包，也可检查固定上游 Release 或选择本地 ZIP。
- 匿名、只读浏览和搜索 Nexus Mods，显示图片、作者、版本、统计与 Nexus ID；Nexus 成人内容会被彻底过滤，不会出现在目录、搜索、详情或缓存结果中。
- v2.2 采用概念图风格的响应式蓝白玻璃界面，提供侧栏、底部状态栏、安全无边框窗口、真实筛选统计和三档视觉回归基线。
...
- `dist/PalDeck-v2.2.2-windows-portable.zip`
- `dist/PalDeck-v2.2.2-windows-portable.zip.sha256`

发布 GitHub Release 时，必须同时上传便携 ZIP 与构建生成的同名 `.sha256`（例如 `PalDeck-v2.2.2-windows-portable.zip` 和 `PalDeck-v2.2.2-windows-portable.zip.sha256`）；缺少或不匹配时应用会安全拒绝更新。构建脚本不会复制文件到桌面。构建完成后可运行正式冒烟脚本；它使用 F 盘 fresh data/TEMP，启动真实 EXE 和 WebView，并通过内部 HTTP 会话验证五个视图、三套主题、三种樱花风格、内置 UE4SS 来源/摘要界面标记、Workshop 空状态及默认背景，全程不写入真实游戏目录：
