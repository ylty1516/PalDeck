...
构建必须从 F 盘的项目工作区执行。脚本固定仓库根目录，将所有临时目录与缓存放在项目内，创建或复用 `.venv-build`，默认按 `requirements-lock.txt` 的固定版本和哈希安装依赖，先运行完整验证，再生成 one-file/windowed 产物：
...
构建脚本不会复制文件到桌面。构建完成后可运行正式冒烟脚本；它使用 F 盘 fresh data/TEMP，启动真实 EXE 和 WebView，并验证内部 HTTP 会话、四页、三主题、樱花及默认背景：

```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\smoke_portable.ps1
```

`PALDECK_SMOKE_REPORT` 仅供该发布自检使用，且只有指向 F 盘文件时才启用；普通启动不设置该变量，行为不受影响。
