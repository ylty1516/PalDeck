...
class TrashService:
    def __init__(
        ...
        *,
        ...
        self.original_roots: dict[str, Path] = {
            ...
            "palschema_classic": Path(
                os.path.abspath(win64 / "Mods" / "PalSchema" / "mods")
            ),
            "palschema_nested": Path(
                os.path.abspath(win64 / "ue4ss" / "Mods" / "PalSchema" / "mods")
            ),
    ...
    def _live_identity(self, manifest: ModManifest, relative_path: str) -> tuple[str, str]:
        if manifest.kind is ModKind.PAK:
            ...
            parent = Path(os.path.abspath(manifest.install_root.parent))
            if parent == self.original_roots["palschema_classic"]:
                root_key = "palschema_classic"
            elif parent == self.original_roots["palschema_nested"]:
                root_key = "palschema_nested"
            else:
        ...
        actual_root = (
            ...
            if manifest.kind not in (ModKind.UE4SS, ModKind.PALSCHEMA)
    ...
    def recycle_local_mod(
        ...
        *,
        ...
        for _payload, original in moves:
            ...
            elif manifest.kind in (ModKind.UE4SS, ModKind.PALSCHEMA):
