...
def is_ue4ss_mod_folder(path: Path) -> bool:
    """Return whether a Mods child has a supported Lua or C++ entry point."""
    return (path / "Scripts" / "main.lua").is_file() or (
        path / "dlls" / "main.dll"
    ).is_file()
...
def get_palschema_mod_roots(game_root: Path | str) -> tuple[Path, Path]:
    """Return PalSchema content-pack roots for both supported UE4SS layouts."""
    return tuple(root / "PalSchema" / "mods" for root in get_ue4ss_mod_roots(game_root))
...
def get_mod_directories(game_root: Path | str) -> dict[str, Path]:
    ...
    return {
        ...
        "palschema_mods": resolve_ue4ss_mods_dir(root) / "PalSchema" / "mods",
        "win64": win64,
