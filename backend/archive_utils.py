...
_PALSCHEMA_CONTENT_FOLDERS = frozenset({
    "appearance", "blueprints", "buildings", "enums", "helpguide", "items",
    "npcs", "paks", "pals", "raw", "resources", "skins", "spawns",
    "translations",
})
...
def _find_ue4ss_root(files: list[tuple[str, ...]]) -> tuple[str, ...] | None:
    for parts in sorted(files, key=lambda item: tuple(part.casefold() for part in item)):
        ...
        if len(parts) >= 2 and folded[-2:] == ("scripts", "main.lua"):
            return parts[:-2]
        if len(parts) >= 2 and folded[-2:] == ("dlls", "main.dll"):
            return parts[:-2]
    return None


def _find_palschema_root(files: list[tuple[str, ...]]) -> tuple[str, ...] | None:
    """Find the mod folder above a recognized PalSchema content directory."""
    candidates: list[tuple[str, ...]] = []
    folded_files = {
        tuple(part.casefold() for part in parts) for parts in files
    }
    for parts in files:
        for index, part in enumerate(parts[:-1]):
            if part.casefold() in _PALSCHEMA_CONTENT_FOLDERS:
                candidate = parts[:index]
                if part.casefold() == "paks":
                    metadata = tuple(
                        component.casefold() for component in (*candidate, "metadata.json")
                    )
                    under_framework = any(
                        parts[offset].casefold() == "palschema"
                        and parts[offset + 1].casefold() == "mods"
                        for offset in range(max(0, index - 3), index - 1)
                    )
                    if metadata not in folded_files and not under_framework:
                        continue
    ...
    if not candidates:
        return None
    unique = {
        tuple(part.casefold() for part in candidate): candidate
        for candidate in candidates
    }
    if len(unique) != 1:
        raise ValueError("ZIP 包含多个 PalSchema 内容模组，请分别打包后导入")
...
def _classify(
    ...
    palschema_root = _find_palschema_root(files)
    ...
    if palschema_root is not None:
        root = dest.joinpath(*palschema_root)
        fallback = Path(palschema_root[-1]).name if palschema_root else "PalSchema Mod"
        return InspectedMod(
            ModKind.PALSCHEMA, _clean_display_name(fallback), root, ()
        )

    if not paks:
        raise ValueError(
            "无法识别该 ZIP：需要 Scripts/main.lua、dlls/main.dll、PalSchema 内容目录或至少一个 .pak 文件，请检查压缩包结构"
        )
